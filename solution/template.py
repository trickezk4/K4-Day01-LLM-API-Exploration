...
def call_openai(
    ...
    lantency = time.time() - start

    return response.choices[0].message.content, lantency
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # 3. Trả về đúng 5 key theo định dạng yêu cầu
    return {
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Bước 3: In từng chunk ngay khi nhận được và gom lại vào biến reply
        print("AI: ", end="", flush=True)
        reply_chunks = []
        
        # DÒNG 364: Vòng lặp for bắt đầu ở đây
        for chunk in stream:
            # Các dòng lệnh bên trong vòng lặp phải thụt lề vào 4 dấu cách (1 tab)
            content = chunk.choices[0].delta.content or ""
            print(content, end="", flush=True)
            reply_chunks.append(content)
            
        print()  # Xuống dòng sau khi in hết các chunk
        reply = "".join(reply_chunks)
        
        # Bước 4: Cập nhật lịch sử hội thoại và cắt giữ lại tối đa 4 lượt (8 messages)
...
def run_assistant(
    ...
    # CHÚ Ý: Import trực tiếp bên trong hàm để chắc chắn không bị lỗi NameError
    from openai import OpenAI
    import os
    import time

    # Bước 1. Khởi tạo trạng thái phiên (Dependency Injection cho get_input)
    if get_input is None:
        get_input = input
    ...
    # Bước 2. Vòng lặp chính - Kiểm tra điều kiện max_turns trước
    while True:
        if max_turns is not None and turns >= max_turns:
            break
        ...
        # Bước 3. Ghép bộ messages luôn có system prompt (persona) đứng đầu
        messages = (
        ...
        # Bước 4. Gọi API qua cơ chế retry bằng hàm lambda
        try:
            ...
            # Bước 5. Gom reply từ stream phát tín hiệu trực tiếp ra màn hình CLI
            print("AI: ", end="", flush=True)
            reply_chunks = []
            ...
            print()  # Xuống dòng khi kết thúc stream một lượt chat
            reply = "".join(reply_chunks)
            
            # Cập nhật bộ nhớ history và giới hạn chặt chẽ 4 lượt (8 messages) gần nhất
            history.append({"role": "user", "content": user_msg})
            history.append({"role": "assistant", "content": reply})
            history = history[-8:]
            
            # Bước 6. Cộng dồn thống kê hiệu năng và chi phí mỗi lượt
            turns += 1
            tokens_used += count_tokens(user_msg, model=OPENAI_MODEL) + count_tokens(reply, model=OPENAI_MODEL)
            total_cost += estimate_cost(user_msg, reply, model=OPENAI_MODEL)["total_cost"]
            
        except Exception as e:
            print(f"\nLỗi xử lý lượt chat: {e}")
            break

    # Bước 7. Trả về dict kết quả gồm đúng 4 key phục vụ test case
    return {
        "turns": turns,
